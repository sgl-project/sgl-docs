...
A working SGLang Diffusion installation with the `sglang` CLI available in your `$PATH`. See the [installation guide](/docs/sglang-diffusion/installation) for setup instructions.
...
For full API usage including image/video generation and LoRA management, see the [OpenAI API documentation](/docs/sglang-diffusion/api/openai-api).
...
| `--tp-size SIZE` | Tensor parallelism size (encoder only; keep ≤ 1 when text encoder offload is enabled) |
...
For example, 5 original frames with `exp=1` → 4 gaps × 1 new frame + 5 originals = **9 frames**; with `exp=2` → **17 frames**.
...
Replace the default VAE with a distilled tiny autoencoder for ~3× faster decoding:
...
Users on the diffusers backend can leverage Cache-DiT acceleration by loading custom cache configs from a YAML file. See the [Cache-DiT documentation](/docs/sglang-diffusion/performance/caching-strategies) for details.
...
See the [environment variables reference](/docs/references/environment_variables) for all storage-related variables.
