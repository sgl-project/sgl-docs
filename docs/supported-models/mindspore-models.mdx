...
MindSpore is a high-performance AI framework optimized for [Ascend NPUs](/docs/hardware-platforms/ascend-npus/SGLang%20installation%20with%20NPUs%20support). This doc guides users to run MindSpore models in SGLang.
...
<Note>Currently, MindSpore models are provided by an independent package `sgl-mindspore`. Support for MindSpore is built upon current SGLang support for Ascend NPU platform. Please first [install SGLang for Ascend NPU](/docs/hardware-platforms/ascend-npus/SGLang%20installation%20with%20NPUs%20support) and then install `sgl-mindspore`.</Note>
