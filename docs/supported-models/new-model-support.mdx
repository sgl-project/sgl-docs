...
<CardGroup cols={2}>
  <Card title="MMMU (Required)" icon="chart-bar">
    Follow the MMMU benchmark [README](https://github.com/sgl-project/sglang/blob/main/benchmark/mmmu/README) to get SGLang vs. HF Transformer accuracy comparison. The accuracy score from SGLang run should not be much lower than that from HF Transformer run. Similarly, follow the [benchmark and profiling guide](../developer-guide/benchmark-and-profiling) to get performance comparison: TTFT and throughput must meet or exceed baselines (e.g., HF Transformer).
  </Card>
