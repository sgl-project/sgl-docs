// DeltaRuleViz — interactive visualization for the Kimi K3 day-0 support blog post.
...
export const DeltaRuleViz = ({ lang = "zh" }) => {
  const PLAYER_UI = {
    play: { zh: "▶ 播放", en: "▶ Play" },
  ...
  const stateAfter = (completed, beta, lang) => {
  ...
  const operation = !current ? { label: lang === "zh" ? "等待 token" : "waiting for a token", detail: "" } : current.kind === "query" ? { label: lang === "zh" ? `qₐ 读出 ${format(readA)}` : `qₐ reads ${format(readA)}`, detail: "o=Sᵀqₐ" } : current.label === "A=4" ? { label: lang === "zh" ? "沿 kₐ 做 delta correction" : "delta-correct along kₐ", detail: `1 + ${beta}×(4−1) = ${format(updatedA)}` } : { label: lang === "zh" ? `首次写入 ${current.label}` : `first write ${current.label}`, detail: "S←S+kvᵀ" };
  return <VizStage
    title={tr(lang, COPY.title)}
    subtitle={tr(lang, COPY.subtitle)}
    player={player}
    lang={lang}
    headExtra={<span className="viz-presets" role="group" aria-label={lang === "zh" ? "A=4 的写入强度" : "write strength for A=4"}>
          {BETAS.map((option) => <button key={option} type="button" className={`viz-btn${beta === option ? " primary" : ""}`} onClick={() => setBeta(option)}>β={option}</button>)}
        </span>}
  >
